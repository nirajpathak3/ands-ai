"""In-process distributed tracing (Day 12, ADR-015).

A tiny, dependency-free tracer: ``start_span`` is a context manager that times a unit of
work, links it to its parent via ``contextvars`` (so nested calls form a tree), records it
in a bounded ring buffer for ``/observability/traces``, and emits one structured JSON log
line per span. When ``OTEL_ENABLED=true`` *and* the OpenTelemetry SDK is installed, each
span is also exported via OTLP — the same seam, upgraded for production with no call-site
changes.

Stdlib only; OpenTelemetry is imported lazily inside the bridge so importing this module
...
_logger = logging.getLogger("secops.trace")
...
class Tracer:
    def __init__(self, *, max_spans: int = 512, otel_export=None) -> None:
...
def build_otel_exporter(settings) -> Any | None:
    ...
    Defensive + lazy: returns None when OTel is disabled or the SDK isn't installed, so the
    in-process tracer is always the working path and OTel is a drop-in production upgrade.
    """
    ...
    try:  # pragma: no cover - exercised only when the OTel SDK is installed
        ...
        tracer = _otel_trace.get_tracer("secops.agent-runtime")
